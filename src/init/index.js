...
const init = async () => {
  ...
  if (boolean) {
    ...
    promiseGetTag().then((tags) => {
      ...
      inquirer.prompt([firstQuestion, secondQuestion]).then(function (answers) {
        ...
        const clone = exec(
          `git clone --progress ${gitUrl} ${projectName}`,
          (error, stdout, stderr) => {
            if (error) {
              console.log(error)
              process.exit()
            ...
            ;+exec(
              `cd ${projectName} && git checkout origin/${answers.line}`,
              (error2, stdout2, stderr2) => {
                ...
                fs.writeFileSync(
                  `${process.cwd()}/${projectName}/package.json`,
                  JSON.stringify(copyData, null, 4),
                  {
                    encoding: 'utf8',
                  }
                )
